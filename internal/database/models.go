...
// ========================
// UNIFIED MULTI-PROJECT MODELS WITH REUSABLE SUBSCRIPTIONS
// ========================
...
// Product represents a product that can be shared across projects or project-specific
type Product struct {
	ID               uuid.UUID `json:"id"`
...
// ProjectProductMapping represents which products are available to which projects
type ProjectProductMapping struct {
	ID        uuid.UUID `json:"id"`
...
// Subscription represents user subscriptions (can span multiple projects)
type Subscription struct {
	ID                   uuid.UUID `json:"id"`
	SubscriptionID       string    `json:"subscription_id"` // Internal subscription identifier
	UserID               string    `json:"user_id"`
...
// SharedProductRequest represents creating a shared product across projects
type SharedProductRequest struct {
	...
	Recurring     bool   `json:"recurring"`
	EligibleProjects []string `json:"eligible_projects,omitempty"` // Specific projects, empty = all
}
...
type ProjectRegistrationRequest struct {
	...
	LocalProducts  []ProductRegistration `json:"local_products"` // Products specific to this project
}
...
type ProjectRegistrationResponse struct {
	...
	LocalProducts  []Product           `json:"local_products"`
	SharedProducts []Product           `json:"shared_products"` // Enabled shared products
	APIKey         string              `json:"api_key"`
	Endpoints      map[string]string   `json:"endpoints"`
	Environment    map[string]string   `json:"environment_variables"`
}

// UserSubscriptionStatus represents user's subscription status across projects
type UserSubscriptionStatus struct {
	UserID        string                    `json:"user_id"`
	Subscriptions []UserActiveSubscription `json:"subscriptions"`
}

// UserActiveSubscription represents an active subscription
type UserActiveSubscription struct {
	SubscriptionID string   `json:"subscription_id"`
	ProductID      string   `json:"product_id"`
	ProductName    string   `json:"product_name"`
	Status         string   `json:"status"`
	EligibleProjects []string `json:"eligible_projects"`
